...
<body>
    <div class="container mt-5">
        ...
        @endif
        <form action="/sedikit/update/{{ $sedikit->id }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="mb-3">
                <label for="judul_kami" class="form-label">judul kami</label>
                <input type="text" class="form-control" id="judul_kami" name="judul_kami"
                    placeholder="masukan judul kami" value="{{ $sedikit->judul_kami }}">
            </div>
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="deskripsi_singkat" name="deskripsi_singkat" placeholder="masukan deskripsi singkat"
                    value="{{ $sedikit->deskripsi_singkat }}">
            </div>
            <div class="form-group">
                <label for="gambar">gambar</label>
                <input type="file" name="gambar" class="form-control" onchange="GambarFoto(event)"><img
                    src="{{ Storage::url($sedikit->gambar) }}" width="100px" target="_blank"
                    id="gambar">
            </div>
